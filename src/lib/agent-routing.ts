...
export function buildAgentRuntimeInstructions({
  ...
  lines.push(
    ...
    "- A Google Slides link or request to inspect or edit a presentation means Shared Google Workspace, not CyWorld Drive and not your private OpenClaw workspace.",
    ...
    "All CyWorld agents can inspect and edit Google Slides only through the shared Google account hjjy.study@gmail.com.",
    "Before editing a presentation, use study_inspect_google_slides. Then use study_update_google_slides with the returned revision ID and native Google Slides batchUpdate requests.",
    "The Google Slides owner must first share the presentation with hjjy.study@gmail.com and grant Editor access. If access is missing, explain this requirement plainly instead of claiming the file is unavailable in general.",
    "Do not describe hjjy.study@gmail.com as your personal Google account. It is one shared CyWorld account used by all agents.",
    "Do not claim a Google Slides change succeeded unless study_update_google_slides returns ok:true. CyWorld records the tool result as a durable action receipt.",
